...
namespace Stocks.CommandLine
{
    class Program
    {
        ...
        void Initialize()
        {
            var dbPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "SVMXLaptopMobile.dll");
        ...
        void ListTableData(string tableName)
        {
            foreach (var resultItem in _db.ExecuteCustomReader(tableName))
            {
                Console.WriteLine(string.Join(" ; ", resultItem.Fields.Select(e => e + ":" + resultItem[e])));
            }
        }
        ...
        void Run()
        {
            ...
            for (;;)
            {
                ...
                else if (cmd == "ls")
                {
                    if (args.Length == 2)
                    {
                        ListTableData(args[1].ToUpperInvariant());
                    }
                    else
                    {
                        ListStocks();
