...
namespace SQLite.Net.Platform.Generic
{
    public class SQLiteApiGeneric : ISQLiteApiExt
    {
        ...
        {
            var internalDbHandle = (DbHandle) db;
            return SQLiteApiGenericInternal.sqlite3_enable_load_extension(internalDbHandle.DbPtr, onoff);
        }
        ...
        public IDbStatement Prepare2(IDbHandle db, string query)
        {
            var internalDbHandle = (DbHandle) db;
            IntPtr stmt;
            Result r = SQLiteApiGenericInternal.sqlite3_prepare_v2(internalDbHandle.DbPtr, query, -1, out stmt, IntPtr.Zero);
            if (r != Result.OK)
            {
        ...
        {
            var internalDbHandle = (DbHandle) db;
            return Marshal.PtrToStringUni(SQLiteApiGenericInternal.sqlite3_errmsg16(internalDbHandle.DbPtr));
        }
        ...
        {
            var internalStmt = (DbStatement) stmt;
            return SQLiteApiGenericInternal.sqlite3_bind_text16(internalStmt.StmtPtr, index, val, n, free);
        }

        public int BindBlob(IDbStatement stmt, int index, byte[] val, int n, IntPtr free)
        {
            var internalStmt = (DbStatement) stmt;
            return SQLiteApiGenericInternal.sqlite3_bind_blob(internalStmt.StmtPtr, index, val, n, free);
        }
